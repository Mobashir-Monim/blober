    ...
    public function run()
    {
        ...
        foreach ($sections as $section) {
            for ($i = 1; $i <= rand(35, 40); $i++) { 
                ...
                DB::table('students')->insert(['user_id' => $user->id,'level_name' => 'Beginner','level' => 0,'points' => 0,'enrollment' => ($now->month <= 4 ? 'Spring ' : ($now->month <= 8 ? 'Summer ' : 'Fall ')) . $now->year,'section' => 1,'student_id' => rand(10000000, 99999999),]);
            }
        }
